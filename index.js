// This in Node file system, but it should be in Express file structure for today
//"npm i -g nodemon" - this installs node globally
...
const PORT = 3000; // in the future you'll put this in an ENV file for security

// import the data from the fake database files
const fruits = require("./data/fruits");
const vegetables = require("./data/vegetables");

// set up the data from the fake database files
...
// for the css styling use this
app.use(express.static("public"));
// app.use(express.static("./styles")); //not this
...
app.get("/", (req, res) => {
  res.send("<div>this is my home</div>"); //It not required to use HTML tags
});
...
app.put("/api/fruits/:id", (req, res) => {
  if (req.params.id >= 0 && req.params.id < fruits.length) {
    ...
    // fruits[req.params.id] = req.body; // uncomment for the PUT
    // res.json(fruits[req.params,id]); // uncomment for the PUT
    if (req.body.readyToEat === "on") {
      // if checked, req.body.readyToEat is defined
    ...
    res.json(`updating ${fruits[req.params.id].name}`); // comment out for the PUT
  } else {
    res.send("<p>That is not a valid id</p>");
  }
});
...
app.put("/api/vegetables/:id", (req, res) => {
  if (req.params.id >= 0 && req.params.id < vegetables.length) {
    res.send(`updating ${vegetables[req.params.id].name}`); // comment out for the PUT
  } else {
    res.send("<p>That is not a valid id</p>");
  }
});
...
app.post("/api/fruits", (req, res) => {
  ...
  // console.log(req.body);
  // need to add logic to change the check or not to true or false
  if (req.body.readyToEat === "on") {
    // if checked, req.body.readyToEat is defined
    req.body.readyToEat = true;
  } else {
...
app.get("/vegetables/:id/edit", (req, res) => {
  if (req.params.id >= 0 && req.params.id < vegetables.length) {
    res.render("vegetables/Edit", { vegetables: vegetables[req.params.id] });
  } else {
    res.send("<p>That is not a valid id</p>");
  }
});
